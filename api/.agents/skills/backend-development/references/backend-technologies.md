...
- **Hono** - Ultrafast, lightweight, multi-runtime (Node.js, Bun, Cloudflare Workers, Deno) with built-in routing, middleware, and type-safe RPC
- **tRPC** - End-to-end typesafe APIs without GraphQL

**When to Choose:** Team already using JavaScript/TypeScript, real-time features needed, rapid development priority, or target deployment is serverless/edge (e.g., using Hono)
...
| Hono | Extreme | Easy | Serverless/Edge, light/fast APIs, multi-runtime |
...
Start → Need real-time features?
       ...
       → No → Need ML/AI integration?
              → Yes → Python + FastAPI
              → No → Target serverless/edge or need ultra-lightweight?
                     → Yes → Node.js/Cloudflare/Bun + Hono
